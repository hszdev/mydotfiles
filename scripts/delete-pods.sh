...
readarray -t podnames < <(kubectl get pods --all-namespaces | grep Terminating | tr -s ' ' | cut -d " " -f 2)
# Read command into array with \n as delimiter
readarray -t namespaces < <(kubectl get pods --all-namespaces | grep Terminating | tr -s ' ' | cut -d " " -f 1)
